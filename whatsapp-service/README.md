...
- **Firewall (Hetzner Cloud, não UFW)**: só portas 22 (SSH) e 3000 (esta API) liberadas, resto negado por padrão — criado via API do Hetzner de propósito (recuperável remotamente mesmo se travar SSH por engano, sem depender da VM estar acessível). A porta 3000 continua aberta pra qualquer IP — a Vercel não tem IP fixo no plano atual, não dá pra restringir por origem. Avaliado usar Cloudflare Tunnel pra fechar isso de vez, mas descartado: trocaria uma dependência simples e direta por uma nova (Cloudflare como intermediário), com uma queda deles passando a impedir a Vercel de mandar mensagem (a sessão do WhatsApp em si não passaria por lá, só o comando de envio).
- **Bloqueio de força bruta no token**: IP que errar `API_TOKEN` 10x em 5min fica bloqueado por 30min (429), mesmo com o token certo depois — 2ª camada de defesa já que a porta fica aberta pro mundo. Normaliza o prefixo `::ffff:` do IP antes de contar (sem isso o mesmo IPv4 podia contar como 2 IPs diferentes e dobrar o limite real).
