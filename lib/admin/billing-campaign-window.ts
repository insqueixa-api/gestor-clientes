...
export const MAX_DELAY_SECS = 5 * 60;

export function normalizeBillingDelayWindow(minSecs: number, maxSecs: number) {
  const normalizedMin = Number.isFinite(minSecs) ? Math.max(MIN_DELAY_SECS, Math.floor(minSecs)) : MIN_DELAY_SECS;
  const normalizedMax = Number.isFinite(maxSecs) ? Math.max(MIN_DELAY_SECS, Math.floor(maxSecs)) : MAX_DELAY_SECS;
