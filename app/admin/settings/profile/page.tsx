...
try {
  ...
  const ok = await confirm({
    ...
    subtitle: "A sessão 1 será encerrada e parará de enviar mensagens.",
    tone: "rose",
...
async function handleReconnectWhatsApp() {
  const ok = await confirm({
    ...
    subtitle: "A sessão 1 será reiniciada e o QR Code gerado novamente.",
    tone: "amber",
...
function WhatsAppSession2Panel({
  ...
  const { confirm } = useConfirm(); // ✅ Inicializado para a Sessão 2
  const [waLoading, setWaLoading]            = useState(false);
...
async function refreshPanel(forceQr = false, showVisualLoading = true) { 
  ...
  async function handleDisconnect() {
    const ok = await confirm({
      ...
      subtitle: "A 2ª sessão do WhatsApp será encerrada.",
      tone: "rose",
      confirmText: "Desconectar",
      cancelText: "Voltar"
    });
...
async function handleReconnect() {
    const ok = await confirm({
      title: "Forçar reconexão?",
      subtitle: "A sessão 2 será reiniciada.",
      tone: "amber",
      confirmText: "Reconectar",
      cancelText: "Voltar"
    });
