...
function WhatsAppSessionCard({
  ...
  async function fetchConfig() {
    try {
      ...
      if (res.ok) {
        ...
        if (apiSuffix === "2") {
          try {
            const s1Res = await fetch("/api/whatsapp/config", {
              cache: "no-store",
            });
            ...
            if (effectiveRedirectEnabled && !s1BotEnabled) {
              ...
              addToast(
                ...
                "O Bot de Atendimento da Sessão 1 está desligado — desativei o redirecionamento da Sessão 2 automaticamente.",
              );
            }
          } catch {}
        }
  ...
  async function handleToggleRedirect() {
    ...
    if (next && apiSuffix === "2" && !session1BotEnabled) {
      addToast(
        "error",
        "Ative o Bot de Atendimento da Sessão 1 primeiro",
        "Não faz sentido redirecionar pra lá com o bot de lá desligado — o cliente ficaria sem resposta nenhuma.",
      );
      return;
    }
  ...
  function startEditRedirectMessage() {
    setDraftRedirectMessage(redirectMessage);
    setEditingRedirectMessage(true);
  }
  async function confirmEditRedirectMessage() {
    setRedirectMessage(draftRedirectMessage);
    setEditingRedirectMessage(false);
    await saveConfig({ redirectMessage: draftRedirectMessage });
  }
            ...
            <div className="flex-1 min-w-0 space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl border border-border">
                <span className="text-sm font-medium text-foreground/90 flex items-center gap-1.5">
                  <span
                    className={`text-base leading-none ${botEnabled ? "" : "opacity-40"}`}
                  >
                    🤖
                  </span>
                  Bot de Atendimento
                ...
                >
                  <span
                    className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-card shadow-sm transition-transform ${botEnabled ? "translate-x-5" : ""}`}
                  />
                </button>
              </div>
              {apiSuffix === "2" && (
                <div className="p-3 rounded-xl border border-border space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-foreground/90 flex items-center gap-1.5">
                      <span
                        className={`text-base leading-none ${redirectEnabled ? "" : "opacity-40"}`}
                      >
                        ↪️
                      </span>
                    ...
                    <button
                      ...
                      disabled={
                        savingConfig ||
                        (apiSuffix === "2" && !session1BotEnabled)
                      }
                      title={
                        apiSuffix === "2" && !session1BotEnabled
                          ? "Ative o Bot de Atendimento da Sessão 1 primeiro"
                    ...
                    >
                      <span
                        className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-card shadow-sm transition-transform ${redirectEnabled ? "translate-x-5" : ""}`}
                      />
                    </button>
                  </div>
                  <p className="text-[10px] text-muted-foreground">
                    Quando ativado, o bot não atende por aqui — só manda o aviso
                    abaixo (pelo número da Sessão 1) e mais nada.
                  </p>
                  {!session1BotEnabled && (
                    <p className="text-[10px] text-rose-500">
                      ⚠️ O Bot de Atendimento da Sessão 1 está desligado —
                      ative-o lá primeiro pra poder habilitar o redirecionamento
                      aqui.
                    </p>
                  )}
                  {redirectEnabled &&
                    (!editingRedirectMessage ? (
                      <div
                        onClick={startEditRedirectMessage}
                        className="px-3 py-2 text-xs bg-transparent border border-dashed border-border rounded-xl text-foreground/80 cursor-pointer hover:border-amber-500/50 whitespace-pre-wrap"
                      ...
                      <div className="space-y-2">
                        <textarea
                          value={draftRedirectMessage}
                          onChange={(e) =>
                            setDraftRedirectMessage(e.target.value)
                          }
                          rows={4}
                          autoFocus
                          className="w-full px-3 py-2 text-xs bg-card border border-amber-500/50 rounded-xl outline-none resize-none"
                        />
                        <div className="flex gap-2">
                          <button
                            onClick={() => setEditingRedirectMessage(false)}
                            className="flex-1 py-1.5 rounded-lg border border-border text-[10px] font-medium text-muted-foreground"
                          >
                            Cancelar
                          </button>
                          <button
                            onClick={() => void confirmEditRedirectMessage()}
                            disabled={savingConfig}
                            className="flex-1 py-1.5 rounded-lg bg-amber-600 text-white text-[10px] font-bold"
                          >
...
function KnowledgeBase({
  ...
  const [editPortalCategory, setEditPortalCategory] = useState<
    "faq" | "device_setup"
  >("faq");
  ...
  async function loadItems() {
    ...
    try {
      ...
      const res = await fetch(`/api/whatsapp/bot/knowledge?${params}`, {
  ...
  async function handleSave() {
    ...
    if (editPortalVisible && !editPortalContent.trim()) {
      addToast(
        "error",
        "Texto pro cliente obrigatório",
        "Pra ficar visível no portal, preencha o texto client-facing.",
      );
      return;
    }
    setSaving(true);
      ...
      if (isNew) {
        const res = await fetch("/api/whatsapp/bot/knowledge", {
          ...
          body: JSON.stringify({
            ...
            content: editContent,
            applies_to_servers: editAppliesTo,
            ...portalFields,
          }),
        });
        const json = await res.json();
        ...
        addToast(
          "success",
          "Conhecimento salvo!",
          json.embedding_generated
            ? "Embedding gerado com sucesso."
            : "Salvo sem embedding — verifique a API key.",
        );
      } else {
        const res = await fetch("/api/whatsapp/bot/knowledge", {
          method: "PUT",
  ...
  async function handleDelete(item: KnowledgeItem) {
    const ok = await confirm({
      ...
      subtitle: `"${item.title}" será deletado permanentemente.`,
      tone: "rose",
      confirmText: "Remover",
      cancelText: "Cancelar",
    });
    ...
    try {
      const token = await getToken();
      const res = await fetch(`/api/whatsapp/bot/knowledge?id=${item.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                {items.length} item{items.length !== 1 ? "s" : ""} · busca
                semântica ativa
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={openNew}
              className="h-9 px-4 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold flex items-center gap-2 shadow-sm transition-all"
            >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`w-1.5 h-1.5 rounded-full shrink-0 ${item.is_active ? "bg-emerald-500" : "bg-muted-foreground/40"}`}
                          />
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-violet-500 bg-violet-500/10 px-1.5 py-0.5 rounded font-medium whitespace-nowrap shrink-0">
                            {item.category}
                          </span>
                          {item.applies_to_servers !== null &&
                            item.applies_to_servers !== undefined &&
                            item.applies_to_servers.length <
                              SERVER_OPTIONS.length && (
                              <span
                                title={
                                  item.applies_to_servers.length === 0
                                    ? "Não aparece pra nenhum servidor — confira as marcações"
                                    : `Só aparece pra clientes de: ${item.applies_to_servers.join(", ")}`
                                }
          ...
          <div className="flex flex-col">
            {!selected && !isNew ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
                ...
                <p className="text-xs">
                  ou crie um novo conhecimento para o bot
                </p>
                <button
                  onClick={openNew}
                  className="mt-4 h-9 px-4 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold flex items-center gap-2"
                >
              ...
              <div className="flex-1 flex flex-col p-5 gap-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-foreground">
                    {isNew ? "Novo conhecimento" : "Editar"}
                  </h3>
                  <button
                    onClick={closeEditor}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                      <Edit3 className="w-3 h-3" /> Assunto / Título
                    </label>
                    <input
                      value={editTitle}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                    Aplica-se a (deixe tudo desmarcado = todos os servidores)
                  </label>
                  <p className="text-[10px] text-muted-foreground/70">
                    Marque só se esse conteúdo for específico de um servidor
                    (ex: código de ativação diferente por servidor) — o bot só
                    mostra pra clientes desses servidores.
                  </p>
                  <div className="flex flex-wrap gap-3 pt-0.5">
                    {SERVER_OPTIONS.map((s) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={editAppliesTo.includes(s.value)}
                          onChange={() => toggleEditServer(s.value)}
                        />
                        {s.label}
                      </label>
                    ))}
                ...
                <div className="space-y-2 p-3 bg-violet-500/5 rounded-lg border border-violet-500/20">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <span
                        className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-card shadow-sm transition-transform ${editPortalVisible ? "translate-x-5" : ""}`}
                      />
                    </button>
                  </div>
                  <p className="text-[10px] text-muted-foreground/70">
                    ...
                    <div className="space-y-2 pt-1">
                      <div className="flex gap-3">
                        <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                          <input
                            type="radio"
                            checked={editPortalCategory === "faq"}
                            onChange={() => setEditPortalCategory("faq")}
                          />
                        ...
                        <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                          <input
                            type="radio"
                            checked={editPortalCategory === "device_setup"}
                            onChange={() =>
                              setEditPortalCategory("device_setup")
                            }
                          />
                        ...
                        <div className="flex flex-wrap gap-3">
                          {ALL_DEVICE_TYPES.map((dt) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={editPortalDeviceTypes.includes(dt)}
                                onChange={() => toggleEditPortalDeviceType(dt)}
                              />
                ...
                {!isNew && (
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg border border-border">
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs font-medium ${editActive ? "text-emerald-500" : "text-muted-foreground"}`}
                      >
                        {editActive
                          ? "Ativo — bot usa este conhecimento"
                          : "Inativo — bot ignora este item"}
                      </span>
                      <button
                      ...
                      >
                        <span
                          className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-card shadow-sm transition-transform ${editActive ? "translate-x-5" : ""}`}
                        />
                      </button>
                    </div>
                    <button
                ...
                <div className="flex gap-3">
                  <button
                    onClick={closeEditor}
                    className="flex-1 h-10 rounded-xl border border-border text-xs font-medium text-muted-foreground hover:bg-muted transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 h-10 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-sm"
                  >
                    {saving ? (
                      <>
                      ...
                      <>
                        <Sparkles className="w-3.5 h-3.5" /> Salvar e Indexar
                      </>
                    )}
                  </button>
                </div>
                ...
                <p className="text-[10px] text-muted-foreground/60 text-center">
                  O conteúdo é convertido em vetor semântico e indexado
                  automaticamente para busca inteligente.
                </p>
              </div>
            )}
          </div>
...
function FloatingChat({
  ...
  const [clients, setClients] = useState<
    { id: string; display_name: string; whatsapp_username?: string | null }[]
  >([]);
  ...
  async function flushAndSend() {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      const responses: string[] = Array.isArray(json.responses)
        ? json.responses
        : [];
      if (json.ok && responses.length > 0) {
        // ✅ Uma bolha por mensagem (Msg 1, Msg 2...), igual chega separado
        // no WhatsApp de verdade — antes juntava tudo numa bolha só com
        // "\n\n", escondendo bug real de nó que manda mensagens demais/
        // faltando (achado 08/08/2026). `meta` (debug) só na última bolha.
        setMessages((prev) => [
          ...prev,
        ...
        setMessages((prev) => [
          ...prev,
          { role: "bot", text: `❌ ${json.error || "sem resposta"}` },
        ]);
      }
    } catch (e: any) {
      setMessages((prev) => [
          ...
          <div className="px-4 py-3 border-b border-border bg-gradient-to-r from-violet-600/10 to-indigo-600/10 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              {messages.length > 0 && (
                <button
                  onClick={clearChat}
                  className="text-[10px] text-rose-500 hover:underline px-2 py-1"
                >
                  Limpar
                </button>
              )}
              ...
              {showDropdown && (
                <div className="absolute z-20 top-8 left-0 right-0 bg-card border border-border rounded-xl shadow-xl max-h-40 overflow-y-auto">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-[11px] hover:bg-muted text-muted-foreground"
                  >
                  ...
                  {clients
                    ...
                    .filter(
                      (c, i, a) =>
                        a.findIndex(
                          (x) => x.whatsapp_username === c.whatsapp_username,
                        ) === i,
                    )
                    .filter((c) => {
                    ...
                    .map((c, i) => (
                      <button
                        ...
                        onMouseDown={() => {
                          ...
                          setSelectedLabel(
                            `${c.display_name} — ${c.whatsapp_username}`,
                          );
                          setClientSearch("");
                          setShowDropdown(false);
                          clearChat();
                        }}
                        className="w-full text-left px-3 py-2 text-[11px] hover:bg-muted border-t border-border/50"
                      >
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {messages.length === 0 && (
              <div className="flex items-center justify-center h-full text-[11px] text-muted-foreground text-center px-4 leading-relaxed">
                Simule mensagens de clientes.
                <br />
                <span className="text-violet-500">Salve boas respostas</span> na
                base de conhecimento
                <br />
                ou <span className="text-rose-500">corrija erros</span> para
                treinar o bot.
              </div>
            )}
            {messages.map((msg, i) => (
              ...
              >
                <div className="max-w-[85%] space-y-1">
                  <div
                    className={`px-3 py-2 rounded-xl text-[11px] whitespace-pre-wrap leading-relaxed ${msg.role === "user" ? "bg-violet-600 text-white rounded-br-sm" : "bg-muted border border-border text-foreground rounded-bl-sm"}`}
                  >
                    {msg.text}
                  </div>
                  ...
                  {msg.role === "bot" && (
                    <div className="flex gap-2 pl-1">
                      <button
                        onClick={() => saveToKnowledge(msg.text)}
                        disabled={savingItem === msg.text}
                        className="text-[10px] text-muted-foreground hover:text-violet-500 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => {
                          ...
                          saveFeedback(lastUser, msg.text);
                        }}
                        className="text-[10px] text-rose-500/70 hover:text-rose-500 transition-colors flex items-center gap-1"
                      >
          ...
          <div className="border-t border-border p-2.5 flex flex-col gap-1.5 bg-muted/10 shrink-0">
            {pendingCount > 0 && (
              <div className="flex items-center justify-between px-0.5">
                <span className="text-[10px] text-muted-foreground">
                  {pendingCount} mensagem{pendingCount > 1 ? "ns" : ""}{" "}
                  aguardando envio...
                </span>
                <button
                  onClick={() => void flushAndSend()}
                  className="text-[10px] text-violet-500 hover:underline font-medium"
                >
                  Enviar agora
                </button>
              </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (bufferRef.current.length > 0 && e.target.value.trim())
                    resetAdminTimers();
                }}
                onKeyDown={(e) => {
              ...
              <button
                ...
                disabled={!input.trim()}
                className="h-9 px-4 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-[11px] font-bold disabled:opacity-50 transition-all"
              >
                Enviar
              </button>
            </div>
    ...
    <div className="space-y-6 pt-0 pb-24 px-0 sm:px-6 min-h-screen bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-3 sm:px-0">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Sessões, base de conhecimento e simulador do bot.
          </p>
        </div>
        <button
      ...
      <div className="bg-card border border-border rounded-2xl p-4 sm:p-5">
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Menus, submenus e respostas cadastradas — cobre a maior parte dos
            atendimentos sem gastar tokens do Gemini.
          </p>
        </div>
        <BotMenuTreeEditor />
      </div>
