...
export default function CupomModal({
  ...
  const [targetStatus, setTargetStatus] = useState<string[]>(coupon?.target_status ?? []);
  const [targetServerIds, setTargetServerIds] = useState<string[]>(coupon?.target_server_ids ?? []);
  const [targetPlanLabels, setTargetPlanLabels] = useState<string[]>(coupon?.target_plan_labels ?? []);
  const [targetAppNames, setTargetAppNames] = useState<string[]>(coupon?.target_app_names ?? []);
  ...
  const [minValue, setMinValue] = useState(
    coupon?.rule_days_min != null ? String(Math.abs(coupon.rule_days_min)) : "365",
  );
  ...
  const [maxValue, setMaxValue] = useState(
    coupon?.rule_days_max != null ? String(Math.abs(coupon.rule_days_max)) : "730",
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      const [srvRes, appRes, cliRes] = await Promise.all([
        supabaseBrowser.from("servers").select("id, name").eq("tenant_id", tenantId),
        supabaseBrowser.from("apps").select("id, name").eq("tenant_id", tenantId),
        supabaseBrowser.from("vw_clients_list_active").select("plan_name").eq("tenant_id", tenantId),
      ]);
      if (!alive) return;
      setAuxServers(((srvRes.data as any[]) || []).map((s) => ({ id: s.id, label: s.name })));
      // target_app_names guarda NOME (a view só expõe nome de app, não id).
      setAuxApps(((appRes.data as any[]) || []).map((a) => ({ id: a.name, label: a.name })));
      const uniquePlans = Array.from(
        new Set(((cliRes.data as any[]) || []).map((c) => c.plan_name).filter(Boolean)),
      );
      setAuxPlans(uniquePlans.map((p) => ({ id: String(p), label: String(p) })));
    })();
    return () => {
      alive = false;
    };
  ...
  function resolveDateRule() {
    if (!hasDateRule) return { ruleDateField: null, ruleDaysMin: null, ruleDaysMax: null } as const;
    return {
      ruleDateField,
  ...
  const canSave = useMemo(() => {
    ...
    if (hasDateRule) {
      if (hasMin && (!Number.isFinite(Number(minValue)) || Number(minValue) < 0)) return false;
      if (hasMax && (!Number.isFinite(Number(maxValue)) || Number(maxValue) < 0)) return false;
  ...
  async function handleSave() {
    ...
    try {
      ...
      const payload: any = {
        ...
        target_status: !hasPersonalClient && targetStatus.length ? targetStatus : null,
        target_server_ids: !hasPersonalClient && targetServerIds.length ? targetServerIds : null,
        target_plan_labels: !hasPersonalClient && targetPlanLabels.length ? targetPlanLabels : null,
        target_app_names: !hasPersonalClient && targetAppNames.length ? targetAppNames : null,
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-border bg-card shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div className="rounded-xl border border-border bg-transparent px-3 py-2.5 space-y-3">
            <div>
              <div className="text-xs font-medium text-foreground/90">Segmentação</div>
              <div className="text-[11px] text-muted-foreground">
                Vazio = sem filtro nessa dimensão (todos passam). Status vazio = Ativo + Vencido (Teste não entra sem escolher).
              </div>
            </div>
          ...
          <div className="rounded-xl border border-border bg-transparent px-3 py-2.5 space-y-2.5">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="text-xs font-medium text-foreground/90">Janela de dias</div>
                <div className="text-[11px] text-muted-foreground">
                  Ex: cadastro entre 1 e 2 anos, ou vencimento entre 3 e 10 dias.
                </div>
              </div>
              <button
                type="button"
            ...
            {hasDateRule && (
              <div className="space-y-3">
                <div>
                  ...
                  <select
                    value={ruleDateField}
                    onChange={(e) => setRuleDateField(e.target.value as "vencimento" | "cadastro")}
                ...
                <p className="text-[11px] text-foreground/70">
                  "Antes" = dias antes da data (ainda não chegou). "Depois" = dias depois (já passou).
                </p>
              </div>
            )}
          </div>
    ...
    <div className="relative" ref={containerRef}>
      ...
      <button
        ...
        className={`w-full h-9 px-3 text-left rounded-lg border text-xs flex justify-between items-center transition-all ${
          open ? "border-emerald-500 ring-1 ring-emerald-500/20" : "border-border bg-transparent text-foreground/90"
        }`}
      >
        <span className={selected.length === 0 ? "text-muted-foreground italic" : "font-medium"}>
          {getLabel()}
        </span>
              ...
              >
                <div
                  className={`w-4 h-4 rounded border flex items-center justify-center transition-colors shrink-0 ${
                    selected.includes(opt.id) ? "bg-emerald-500 border-emerald-500" : "border-border"
                  }`}
                >
                  {selected.includes(opt.id) && <span className="text-[10px] text-white">✓</span>}
                </div>
                <span className="text-xs text-foreground/90 truncate">{opt.label}</span>
              </div>
            ))}
          </div>
