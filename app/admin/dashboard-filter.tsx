...
export function DashboardFilter({ availableModules }: { availableModules: string[] }) {
  ...
  const currentRaw = searchParams.get("view");
  const activeViews = currentRaw
    ? currentRaw.split(",").filter(v => availableModules.includes(v))
    : availableModules;
  ...
  function toggle(key: string) {
    const next = activeViews.includes(key)
      ...
      : [...activeViews, key];
    const effective = next.length === 0 ? availableModules : next;
  ...
  const label = allSelected
    ? "Mostrar Tudo"
    : activeViews.map(v => MODULE_META[v]?.label ?? v).join(", ");
