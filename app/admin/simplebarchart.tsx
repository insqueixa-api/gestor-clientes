...
const palettes = {
  blue: {
    ...
    glowL: "rgba(59,130,246,0.22)",
    glowHL: "rgba(59,130,246,0.42)",
    glowD: "rgba(96,165,250,0.45)",
    glowHD: "rgba(96,165,250,0.75)",
    accL: "#2563eb",
    accD: "#60a5fa",
    ringL: "rgba(59,130,246,0.5)",
    ringD: "rgba(96,165,250,0.6)",
  },
  emerald: {
    barL: "linear-gradient(to top,#065f46,#059669,#6ee7b7)",
    barD: "linear-gradient(to top,#065f46,#34d399,#a7f3d0)",
    glowL: "rgba(5,150,105,0.18)",
    glowHL: "rgba(5,150,105,0.38)",
    glowD: "rgba(52,211,153,0.4)",
    glowHD: "rgba(52,211,153,0.7)",
    accL: "#059669",
    accD: "#10b981",
    ringL: "rgba(5,150,105,0.5)",
    ringD: "rgba(52,211,153,0.6)",
  },
  violet: {
    barL: "linear-gradient(to top,#4c1d95,#7c3aed,#c4b5fd)",
    barD: "linear-gradient(to top,#4c1d95,#a78bfa,#ede9fe)",
    glowL: "rgba(124,58,237,0.18)",
    glowHL: "rgba(124,58,237,0.38)",
    glowD: "rgba(167,139,250,0.45)",
    glowHD: "rgba(167,139,250,0.75)",
    accL: "#7c3aed",
    accD: "#a78bfa",
    ringL: "rgba(124,58,237,0.5)",
    ringD: "rgba(167,139,250,0.6)",
  },
  rose: {
    barL: "linear-gradient(to top,#881337,#e11d48,#fda4af)",
    barD: "linear-gradient(to top,#881337,#fb7185,#ffe4e6)",
    glowL: "rgba(225,29,72,0.18)",
    glowHL: "rgba(225,29,72,0.38)",
    glowD: "rgba(251,113,133,0.45)",
    glowHD: "rgba(251,113,133,0.75)",
    accL: "#e11d48",
    accD: "#f43f5e",
    ringL: "rgba(225,29,72,0.5)",
    ringD: "rgba(251,113,133,0.6)",
  },
  amber: {
    barL: "linear-gradient(to top,#92400e,#d97706,#fcd34d)",
    barD: "linear-gradient(to top,#78350f,#fbbf24,#fef3c7)",
    glowL: "rgba(217,119,6,0.18)",
    glowHL: "rgba(217,119,6,0.38)",
    glowD: "rgba(251,191,36,0.45)",
    glowHD: "rgba(251,191,36,0.75)",
    accL: "#d97706",
    accD: "#fbbf24",
    ringL: "rgba(217,119,6,0.5)",
    ringD: "rgba(251,191,36,0.6)",
  },
};
...
export function SimpleBarChart({
  ...
  const { maxNice, ticks } = useMemo(() => {
    ...
    return {
      maxNice,
      ticks: Array.from({ length: 5 }, (_, i) => (maxNice * i) / 4),
    };
  }, [data]);
    ...
    <div className="w-full select-none">
      ...
      >
        ...
        >
          ...
          <div
            className="relative"
            style={{ borderRight: `1px solid ${borderY}` }}
          >
            {ticks.map((t, i) => (
              <div
                key={i}
                className="absolute left-0 w-full"
                style={{ bottom: `calc(${(i / 4) * 100}% + 1.75rem)` }}
              >
                <span
          ...
          <div className="relative px-3 pt-3 pb-7">
            {ticks.map((_, i) => (
              <div
                ...
                style={{
                  bottom: `calc(${(i / 4) * 100}% + 1.75rem)`,
                  borderTop: `1px ${i === 0 ? "solid" : "dashed"} ${i === 0 ? gridSolid : gridDash}`,
                }}
              />
            ))}
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        ...
                        transition:
                          "height .5s cubic-bezier(.22,.68,0,1.2),box-shadow .2s,opacity .2s",
                        opacity: isZero ? 0.3 : isH ? 1 : 0.86,
                        boxShadow: isH
                          ? `0 0 12px 2px ${ring},inset 0 1px 0 rgba(255,255,255,${sheenAlpha})`
                          : `inset 0 1px 0 rgba(255,255,255,${sheenAlpha})`,
                        zIndex: 1,
                      }}
                    >
                      {!isZero && (
                        <div
                          ...
                          style={{
                            background: `linear-gradient(to bottom,rgba(255,255,255,${sheenAlpha}),transparent)`,
                          }}
                        />
                      )}
                    </div>
        ...
        {hoveredItem && (
          <div
            ...
            style={{
              left: tipLeft,
              top: tipTop < 0 ? tooltipPos.y + 12 : tipTop,
              width: TIP_W,
            }}
          >
            <div
            ...
            >
              <div
                ...
                style={{ fontSize: "11px", color: acc }}
              >
                {hoveredItem.tooltipTitle}
              </div>
              <div
                className="mt-0.5"
                style={{ fontSize: "12px", color: tipText }}
              >
