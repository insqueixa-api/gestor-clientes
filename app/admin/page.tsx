...
export default async function AdminDashboardPage({
  ...
  const isSaasRole = myRole === "SUPERADMIN" || myRole === "MASTER";
  ...
  const hasSaaS       = tenantModules.includes("saas") && isSaasRole;
  const hasFinanceiro = tenantModules.includes("financeiro");

  // Módulos disponíveis para este usuário
  const availableModules = ["iptv", "saas", "financeiro"].filter(m =>
    m === "iptv" ? hasIPTV : m === "saas" ? hasSaaS : hasFinanceiro
  );

  // Filtro via URL — se vier vazio ou "all", mostra tudo disponível
  const paramView = resolvedParams?.view;
  const requestedViews = paramView
    ? paramView.split(",").filter(v => availableModules.includes(v))
    : [];
  // Se nenhum selecionado ou "all", usa todos disponíveis
  const activeViews = requestedViews.length > 0 ? requestedViews : availableModules;
  ...
  // Helper para href do chip toggle
  function chipHref(key: string): string {
    const isActive = activeViews.includes(key);
    const allActive = activeViews.length === availableModules.length;

    // Se todos estão ativos e clicou num: filtra só os outros
    if (allActive) {
      const next = availableModules.filter(v => v !== key);
      if (next.length === 0) return "/admin"; // proteção: nunca zera
      return `/admin?view=${next.join(",")}`;
    }

    // Se clicou no que está ativo: remove ele
    if (isActive) {
      const next = activeViews.filter(v => v !== key);
      if (next.length === 0) return "/admin"; // volta para tudo
      return `/admin?view=${next.join(",")}`;
    }

    // Se clicou no que está inativo: adiciona ele
    const next = [...activeViews, key];
    if (next.length === availableModules.length) return "/admin"; // tudo = sem param
    return `/admin?view=${next.join(",")}`;
  }
  ...
  const [
    ...
    (hasSaaS && myTenantId
      ...
      : Promise.resolve({ data: null })) as Promise<any>,
    (myTenantId
      ? supabase.from("server_credit_purchases").select("created_at, total_amount_brl").eq("tenant_id", myTenantId)
          .gte("created_at", isoDateFromYMD(
            new Date(_finYear, _finMonth - 2, 1).getFullYear(),
            new Date(_finYear, _finMonth - 2, 1).getMonth() + 1,
            1
          ))
      : Promise.resolve({ data: null })) as Promise<any>,
    (hasSaaS && myTenantId
      ? supabase.from("saas_credit_transactions").select("created_at, price_amount")
          .eq("tenant_id", myTenantId).in("type", ["purchase", "grant"])
      : Promise.resolve({ data: null })) as Promise<any>,
  ]);
  ...
  for (const row of saasCostRows) {
    ...
    if (sy === todayY && sm === todayM && sd === todayD) saasCostTodayVal += amt;
    if (sy === todayY && sm === todayM) saasCostMonthVal += amt;
    if (sy === prevY  && sm === prevM)  saasCostPrevMonthVal += amt;
  }

  // Finanças Pessoais
  type FinTrx = {
    id: string; tipo: "RECEITA" | "DESPESA"; valor: number;
    status: string; data_vencimento: string; data_pagamento: string | null; categoria_id: string | null;
  };
  let finTrxRows: FinTrx[] = [];
  const finCatById = new Map<string, { nome: string; icone: string }>();
  let finSaldoAtual = 0;

  if (myTenantId && hasFinanceiro) {
    const _finNextMonthStart = isoDateFromYMD(
      _finMonth === 12 ? _finYear + 1 : _finYear,
      _finMonth === 12 ? 1 : _finMonth + 1, 1
    );
    const [trxRes, catRes] = await Promise.allSettled([
      ...
      supabase.from("fin_categorias").select("id, nome, icone").eq("tenant_id", myTenantId),
    ]);
    ...
    const contasRes = await supabase.from("fin_contas_bancarias").select("id").eq("tenant_id", myTenantId);
    if (contasRes.data && contasRes.data.length > 0) {
      ...
      for (const s of saldos) {
        if (s.status === "fulfilled" && !s.value.error) finSaldoAtual += toNumber(s.value.data);
      }
    }
  }
  ...
  const finReceitasPagas = finTrxRows.filter(t => t.tipo === "RECEITA" && isFinPagoNoMes(t)).reduce((a, t) => a + toNumber(t.valor), 0);
  const finDespesasPagas = finTrxRows.filter(t => t.tipo === "DESPESA" && isFinPagoNoMes(t)).reduce((a, t) => a + toNumber(t.valor), 0);
  const finReceitasTotal = finTrxRows.filter(t => t.tipo === "RECEITA" && t.data_vencimento >= _finMonthStart && t.data_vencimento <= _finMonthEnd).reduce((a, t) => a + toNumber(t.valor), 0);
  const finDespesasTotal = finTrxRows.filter(t => t.tipo === "DESPESA" && t.data_vencimento >= _finMonthStart && t.data_vencimento <= _finMonthEnd).reduce((a, t) => a + toNumber(t.valor), 0);
  const finReceitasPendentes = finTrxRows.filter(t => t.tipo === "RECEITA" && t.status !== "PAGO" && t.data_vencimento >= _finMonthStart && t.data_vencimento <= _finMonthEnd).reduce((a, t) => a + toNumber(t.valor), 0);
  const finDespesasPendentes = finTrxRows.filter(t => t.tipo === "DESPESA" && t.status !== "PAGO" && t.data_vencimento >= _finMonthStart && t.data_vencimento <= _finMonthEnd).reduce((a, t) => a + toNumber(t.valor), 0);
  ...
  const finCatRevenueItems: BarItem[] = Array.from(catRevMap.values()).sort((a, b) => b.value - a.value).slice(0, 5);
  const finCatExpenseItems: BarItem[] = Array.from(catExpMap.values()).sort((a, b) => b.value - a.value).slice(0, 5);
  ...
  for (const row of dueRows) {
    ...
    dueByOffset.set(off, { qty: toNumber(row.qty), amount: toNumber(row.amount_brl_estimated) });
  }
  ...
  for (const r of regsRows) {
    ...
    regsMap.set(key, { clients: toNumber(r.clients_created), trials: toNumber(r.trials_created) });
  }
  const chartRegsData: SimpleBarChartDatum[] = daysFromMonthStartToTodaySP().map(({ iso, dayNum }) => {
    ...
    return { label: String(dayNum), value: total, displayValue: total, tooltipTitle: spTitleFromISO(iso), tooltipContent: `${fmtInt(found.clients)} Clientes / ${fmtInt(found.trials)} Testes` };
  });

  const payMap = new Map<string, { clients: number; reseller: number }>();
  for (const r of paymentsRows) {
    const key = normalizeDayKey(r.day);
    payMap.set(key, { clients: toNumber(r.clients_paid_brl_estimated), reseller: toNumber(r.reseller_paid_brl) });
  }
  const chartPaymentsData: SimpleBarChartDatum[] = daysFromMonthStartToTodaySP().map(({ iso, dayNum }) => {
    ...
    return { label: String(dayNum), value: totalVal, displayValue: totalVal, tooltipTitle: spTitleFromISO(iso), tooltipContent: `Clientes: ${fmtBRL(found.clients)} • Revenda: ${fmtBRL(found.reseller)} • Total: ${fmtBRL(totalVal)}` };
  });

  const saasDailyMap = new Map<string, { renewal: number; credits: number; resellers: number }>();
  for (const r of saasDailyRows) {
    const key = normalizeDayKey(r.day);
    saasDailyMap.set(key, { renewal: toNumber(r.renewal_brl), credits: toNumber(r.credits_brl), resellers: toNumber(r.new_resellers) });
  }
  const chartSaasRevenueData: SimpleBarChartDatum[] = daysFromMonthStartToTodaySP().map(({ iso, dayNum }) => {
    ...
    return { label: String(dayNum), value: total, displayValue: total, tooltipTitle: spTitleFromISO(iso), tooltipContent: `Renovações: ${fmtBRL(found.renewal)} • Créditos: ${fmtBRL(found.credits)} • Total: ${fmtBRL(total)}` };
  });
  const chartSaasResellersData: SimpleBarChartDatum[] = daysFromMonthStartToTodaySP().map(({ iso, dayNum }) => {
    const found = saasDailyMap.get(iso) ?? { renewal: 0, credits: 0, resellers: 0 };
    return { label: String(dayNum), value: found.resellers, displayValue: found.resellers, tooltipTitle: spTitleFromISO(iso), tooltipContent: `${fmtInt(found.resellers)} novo(s) revendedor(es)` };
  });

  const topServersItems: BarItem[] = topServers.map(s => ({ label: s.server_name, value: toNumber(s.clients_created) }));
  const topAppsItems: BarItem[]    = topApps.map(a => ({ label: a.app_name, value: toNumber(a.clients_count) }));

  // Labels/cores dos chips
  const MODULE_CHIP: Record<string, { label: string; activeClass: string; inactiveClass: string }> = {
    iptv: {
      label: "📺 IPTV",
      activeClass: "bg-sky-500 border-sky-500 text-white",
      inactiveClass: "bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/40 hover:border-sky-300 hover:text-sky-600",
    },
    saas: {
      label: "⚡ SaaS",
      activeClass: "bg-violet-500 border-violet-500 text-white",
      inactiveClass: "bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/40 hover:border-violet-300 hover:text-violet-600",
    },
    financeiro: {
      label: "📊 Financeiro",
      activeClass: "bg-emerald-500 border-emerald-500 text-white",
      inactiveClass: "bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/40 hover:border-emerald-300 hover:text-emerald-600",
    },
  };
  ...
  return (
    <div id="dashboard-values" className="space-y-6 pt-0 pb-6 px-0 sm:px-6 text-zinc-900 dark:text-zinc-100">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-3 sm:px-0">
        ...
        {/* Chips de filtro — só aparece se o usuário tem mais de 1 módulo */}
        {availableModules.length > 1 && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Link
              href="/admin"
              prefetch={false}
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-bold transition-all shadow-sm ${
                activeViews.length === availableModules.length
                  ? "bg-slate-800 border-slate-700 text-white dark:bg-white/10 dark:border-white/20"
                  : "bg-white dark:bg-zinc-900 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/40 hover:border-slate-400"
              }`}
            >
              Todos
            </Link>
            ...
            {/* Um chip por módulo disponível */}
            {availableModules.map(key => {
              const chip = MODULE_CHIP[key];
              ...
              return (
                <Link
                  ...
                  prefetch={false}
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-bold transition-all shadow-sm ${
                    active && !allActive ? chip.activeClass : chip.inactiveClass
                  }`}
                >
                  {chip.label}
                </Link>
      ...
      {showIPTV && (
        <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-3">
          <MetricCardView title="Ativos" accent="green" leftLabel="Clientes" leftValue={fmtInt(activeClients)} rightLabel="MRR Estimado" rightValue={fmtBRL(activeMrr)} footer="Mês atual" href="/admin/cliente?filter=ativos" />
          <MetricCardView title="Vencidos" accent="red" leftLabel="Clientes" leftValue={fmtInt(overdueClients)} rightLabel="Pendente" rightValue={fmtBRL(overdueAmount)} footer="Mês atual" href="/admin/cliente?filter=vencidos" />
          <MetricCardView title="Testes" accent="blue" leftLabel="Criados" leftValue={fmtInt(trialsCreated)} rightLabel="Conversão" rightValue={fmtPct(trialsConvPct)} footer={`Ativos: ${fmtInt(trialsActive)} • Convertidos: ${fmtInt(trialsConverted)}`} href="/admin/teste" />
        </div>
      )}
      ...
      {showIPTV && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-2 xl:grid-cols-4">
            <MetricCardView
              title="Recebidos Hoje" accent="green"
              leftLabel={`Clientes (${fmtInt(clientsTodayQty)})`}
              leftValue={<><span className="sm:hidden">{fmtBRLNoSymbol(clientsTodayVal)}</span><span className="hidden sm:inline">{fmtBRL(clientsTodayVal)}</span></>}
              rightLabel={`Revenda (${fmtInt(resellerTodayQty)})`}
              rightValue={<><span className="sm:hidden">{fmtBRLNoSymbol(resellerTodayVal)}</span><span className="hidden sm:inline">{fmtBRL(resellerTodayVal)}</span></>}
              footer={<><span className="sm:hidden">Total: {fmtBRLNoSymbol(clientsTodayVal + resellerTodayVal)}</span><span className="hidden sm:inline">Total: {fmtBRL(clientsTodayVal + resellerTodayVal)}</span></>}
            />
            <MetricCardView
              title="Faturamento (Mês)" accent="green"
              leftLabel={`Clientes (${fmtInt(clientsMonthQty)})`}
              leftValue={<><span className="sm:hidden">{fmtBRLNoSymbol(clientsMonthVal)}</span><span className="hidden sm:inline">{fmtBRL(clientsMonthVal)}</span></>}
              rightLabel={`Revenda (${fmtInt(resellerMonthQty)})`}
              rightValue={<><span className="sm:hidden">{fmtBRLNoSymbol(resellerMonthVal)}</span><span className="hidden sm:inline">{fmtBRL(resellerMonthVal)}</span></>}
              footer={
                <div className="flex justify-between items-center w-full">
                  <span><span className="sm:hidden">Total: {fmtBRLNoSymbol(clientsMonthVal + resellerMonthVal)}</span><span className="hidden sm:inline">Total: {fmtBRL(clientsMonthVal + resellerMonthVal)}</span></span>
                  <span className={(clientsMonthVal + resellerMonthVal) - expensesMonthVal < 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-700 dark:text-emerald-400"}>
                    <span className="sm:hidden">Lucro: {fmtBRLNoSymbol((clientsMonthVal + resellerMonthVal) - expensesMonthVal)}</span>
                    <span className="hidden sm:inline">Lucro: {fmtBRL((clientsMonthVal + resellerMonthVal) - expensesMonthVal)}</span>
                  </span>
                </div>
              }
            />
            <MetricCardView
              title="A Receber (Ativos)" accent="amber"
              leftLabel={`Clientes (${fmtInt(toReceiveQty)})`}
              leftValue={<><span className="sm:hidden">{fmtBRLNoSymbol(toReceiveVal)}</span><span className="hidden sm:inline">{fmtBRL(toReceiveVal)}</span></>}
              footer="Até o fim do mês"
            />
            <MetricCardView
              title="Mês Anterior" accent="gray"
              leftLabel={`Clientes (${fmtInt(clientsPrevMonthQty)})`}
              leftValue={<><span className="sm:hidden">{fmtBRLNoSymbol(clientsPrevMonthVal)}</span><span className="hidden sm:inline">{fmtBRL(clientsPrevMonthVal)}</span></>}
              rightLabel={`Revenda (${fmtInt(resellerPrevMonthQty)})`}
              rightValue={<><span className="sm:hidden">{fmtBRLNoSymbol(resellerPrevMonthVal)}</span><span className="hidden sm:inline">{fmtBRL(resellerPrevMonthVal)}</span></>}
              footer={
                <div className="flex justify-between items-center w-full">
                  <span><span className="sm:hidden">Total: {fmtBRLNoSymbol(clientsPrevMonthVal + resellerPrevMonthVal)}</span><span className="hidden sm:inline">Total: {fmtBRL(clientsPrevMonthVal + resellerPrevMonthVal)}</span></span>
                  <span className={(clientsPrevMonthVal + resellerPrevMonthVal) - expensesPrevMonthVal < 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-700 dark:text-emerald-400"}>
      ...
      {showSaasView && (
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-3">
            <MetricCardView
              title="SaaS Recebido Hoje" accent="green"
            ...
            />
            <MetricCardView
              title="SaaS Faturamento (Mês)" accent="green"
            ...
            />
            <MetricCardView
              title="SaaS Mês Anterior" accent="gray"
      ...
      {showFinView && (
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-4">
            <MetricCardView title="💰 Receitas do Mês" accent="green" leftLabel="Recebido no Mês" leftValue={fmtBRL(finReceitasPagas)} rightLabel="A Receber" rightValue={fmtBRL(finReceitasPendentes)} footer={`Previsão total: ${fmtBRL(finReceitasTotal)}`} />
            <MetricCardView title="📉 Despesas do Mês" accent="red" leftLabel="Pago no Mês" leftValue={fmtBRL(finDespesasPagas)} rightLabel="A Pagar" rightValue={fmtBRL(finDespesasPendentes)} footer={`Previsão total: ${fmtBRL(finDespesasTotal)}`} />
            <MetricCardView title="📊 Saldo do Mês" accent={finReceitasPagas - finDespesasPagas >= 0 ? "green" : "red"} leftLabel="Resultado no Mês" leftValue={fmtBRL(finReceitasPagas - finDespesasPagas)} footer={`Previsão: ${fmtBRL(finReceitasTotal - finDespesasTotal)}`} />
            <MetricCardView title="💰 Saldo Atual" accent={finSaldoAtual >= 0 ? "green" : "red"} leftLabel="Saldo em conta" leftValue={fmtBRL(finSaldoAtual)} footer="Atualizar saldo..." href="/admin/settings/financeiro_pessoal" />
          </div>
          {(finCatRevenueItems.length > 0 || finCatExpenseItems.length > 0) && (
            <div className="grid grid-cols-1 gap-3 sm:gap-6 lg:grid-cols-2">
              {finCatRevenueItems.length > 0 && <div className="sv"><RankingCard title="Receitas por Categoria" items={finCatRevenueItems} accentColor="emerald" mode="currency" /></div>}
              {finCatExpenseItems.length > 0 && <div className="sv"><RankingCard title="Despesas por Categoria" items={finCatExpenseItems} accentColor="rose" mode="currency" /></div>}
            </div>
          )}
        </>
      )}
      ...
      {showIPTV && (
        <div className="grid grid-cols-1 gap-3 sm:gap-6 lg:grid-cols-2">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3 sm:p-6 shadow-sm">
            <h3 className="text-base sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2 sm:mb-4">Novos clientes</h3>
            <div className="sv w-full">
              <SimpleBarChart data={chartRegsData} colorClass="from-emerald-400 to-emerald-600 ring-emerald-500" label="Cadastros" heightClass="h-40 sm:h-56" />
              {chartRegsData.length === 0 && <div className="text-zinc-400 text-sm mt-3">Sem dados no mês atual.</div>}
            </div>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3 sm:p-6 shadow-sm">
            <h3 className="text-base sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2 sm:mb-4">Pagamentos Recebidos</h3>
            <div className="sv w-full">
              <SimpleBarChart data={chartPaymentsData} colorClass="from-sky-400 to-blue-600 ring-blue-500" label="BRL" heightClass="h-40 sm:h-56" />
              {chartPaymentsData.length === 0 && <div className="text-zinc-400 text-sm mt-3">Sem dados no mês atual.</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {showSaasView && (
        <div className="grid grid-cols-1 gap-3 sm:gap-6 lg:grid-cols-2">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3 sm:p-6 shadow-sm">
            <h3 className="text-base sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2 sm:mb-4">Receita SaaS (Mês)</h3>
            <div className="sv w-full">
              <SimpleBarChart data={chartSaasRevenueData} colorClass="from-violet-400 to-violet-600" label="BRL" heightClass="h-40 sm:h-56" />
            </div>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3 sm:p-6 shadow-sm">
            <h3 className="text-base sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2 sm:mb-4">Novos Revendedores SaaS</h3>
            <div className="sv w-full">
              <SimpleBarChart data={chartSaasResellersData} colorClass="from-amber-400 to-amber-600" label="Revendas" heightClass="h-40 sm:h-56" />
            </div>
          </div>
        </div>
      )}
...
function VencimentoCard({ diff, map, title, color }: { diff: number; map: Map<number, DueBucket>; title: string; color: Accent }) {
  ...
  return (
    <MetricCardView
      ...
      leftLabel="Qtd" leftValue={fmtInt(d.qty)}
      rightLabel="Valor" rightValue={fmtBRL(d.amount)}
...
function MetricCardView({
  title, accent, leftLabel, leftValue, rightLabel, rightValue, footer, href,
}: {
  title: string; accent: Accent;
  leftLabel: string; leftValue: ReactNode;
  rightLabel?: string; rightValue?: ReactNode;
  footer?: ReactNode; href?: string;
}) {
  const colors: Record<Accent, string> = {
    green:  "border-emerald-200 bg-emerald-50 dark:bg-emerald-950/20 dark:border-emerald-800 text-emerald-900 dark:text-emerald-100",
    red:    "border-rose-200 bg-rose-50 dark:bg-rose-950/20 dark:border-rose-800 text-rose-900 dark:text-rose-100",
    amber:  "border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 text-amber-900 dark:text-amber-100",
    yellow: "border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20 dark:border-yellow-800 text-yellow-900 dark:text-yellow-100",
    blue:   "border-blue-200 bg-blue-50 dark:bg-blue-950/20 dark:border-blue-800 text-blue-900 dark:text-blue-100",
    gray:   "border-zinc-200 bg-white dark:bg-zinc-900 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100",
  };
    ...
    <>
      ...
      <div className="sv p-3 sm:p-4 flex gap-2 sm:gap-4 flex-1">
        <div className="min-w-0 flex-1">
          <div className="text-[9px] sm:text-[10px] uppercase tracking-wider opacity-70 mb-1">{leftLabel}</div>
          <div className="text-[15px] sm:text-xl font-bold leading-tight whitespace-nowrap tabular-nums">{leftValue}</div>
        </div>
        {rightLabel && rightValue && (
          <div className="text-right min-w-0 flex-1">
            <div className="text-[9px] sm:text-[10px] uppercase tracking-wider opacity-70 mb-1">{rightLabel}</div>
            <div className="text-[15px] sm:text-xl font-bold leading-tight whitespace-nowrap tabular-nums">{rightValue}</div>
          </div>
        )}
      </div>
      {footer && <div className="sv px-3 sm:px-4 py-2 text-[11px] sm:text-xs bg-black/5 dark:bg-white/5 opacity-80">{footer}</div>}
    </>
  );

  const baseClass = `rounded-xl border shadow-sm overflow-hidden flex flex-col ${colors[accent]}`;
  if (href) {
    return <Link href={href} target="_blank" className={`${baseClass} hover:scale-[1.02] transition-transform cursor-pointer hover:shadow-md`}>{content}</Link>;
  }
