      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
           {filteredMessages.map((msg) => (
             // CARD COMPACTO (h-40)
             <div key={msg.id} className="group bg-white dark:bg-[#161b22] border border-slate-200 dark:border-white/10 rounded-xl p-4 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between h-40 relative overflow-hidden">
                ...
                <div className="flex justify-center gap-2 mb-3">
                    <button 
                        onClick={() => { setSelectedTemplate(msg); setShowPreview(true); }}
...
// ============================================================================
function PreviewModal({ template, onClose, onEdit }: { template: MessageTemplate, onClose: () => void, onEdit: () => void }) {
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-lg bg-white dark:bg-[#161b22] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
                ...
                <div className="px-5 py-4 border-t border-slate-100 dark:border-white/5 flex justify-end gap-2 bg-white dark:bg-[#161b22]">
                    <button onClick={onClose} className="px-4 py-2 rounded-lg border border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/60 font-bold text-xs hover:bg-slate-50 dark:hover:bg-white/5 transition-colors uppercase">
                        Fechar
                    </button>
                    <button onClick={onEdit} className="px-6 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs shadow-lg shadow-amber-500/20 transition-transform active:scale-95 uppercase flex items-center gap-2">
                        ✏️ Editar Modelo
                    </button>
                </div>
            </div>
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50 dark:bg-white/5">
           <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center text-xl">📝</div>
             <div>
               <h2 className="text-lg font-bold text-slate-800 dark:text-white">{templateToEdit ? "Editar Mensagem" : "Criar Nova Mensagem"}</h2>
               <p className="text-xs text-slate-500 dark:text-white/60">Configure o modelo utilizando as variáveis dinâmicas.</p>
             </div>
           </div>
           <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-800 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">✕</button>
        </div>

        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
            <div className="flex-1 p-6 flex flex-col gap-5 overflow-y-auto custom-scrollbar border-r border-slate-100 dark:border-white/5">
                <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-white/50 uppercase mb-1.5 tracking-wider">Nome do Modelo (Identificação interna)</label>
                    <input 
                        value={name}
                ...
                <div className="flex-1 flex flex-col">
                    <label className="block text-xs font-bold text-slate-500 dark:text-white/50 uppercase mb-1.5 tracking-wider">Conteúdo da Mensagem</label>
            ...
            <div className="w-full lg:w-96 bg-white dark:bg-[#161b22] flex flex-col">
                <div className="p-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                    <h3 className="text-xs font-bold text-slate-600 dark:text-white uppercase tracking-widest flex items-center gap-2">🏷️ Variáveis Disponíveis</h3>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {group.tags.map((tag) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-[10px] opacity-60 group-hover:opacity-100 mt-0.5 font-medium">{tag.desc}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 flex justify-between items-center">
           <div className="text-xs text-slate-400 hidden sm:block">💡 Dica: Use <strong>{`{saudacao_tempo}`}</strong> para enviar "Bom dia" automático.</div>
           <div className="flex gap-3 w-full sm:w-auto justify-end">
              <button onClick={onClose} className="px-6 py-3 rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-white/60 font-bold text-xs hover:bg-white dark:hover:bg-white/10 transition-colors uppercase tracking-wider">Cancelar</button>
              <button onClick={handleSave} disabled={loading} className="px-8 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-lg shadow-emerald-900/20 transition-transform active:scale-95 flex items-center gap-2 uppercase tracking-wider disabled:opacity-50">
