...
export default function ServerDetailsPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const clientIds = [
        ...new Set(
          (renewalsData || []).map((r: any) => r.client_id).filter(Boolean),
        ),
      ];
      ...
      const mappedRenewals: MovementRow[] = (renewalsData || []).map(
        (r: any) => {
          const clientInfo = clientsMap[r.client_id] || {};
          const clientName = clientInfo.display_name || "Cliente Desconhecido";
          const userName = clientInfo.server_username
            ? `(${clientInfo.server_username})`
            : "";
          ...
          if (
            rawNotes.startsWith("Renovação") ||
            rawNotes.startsWith("Renovacao")
          ) {
      ...
      const allMoves = [...(movData || []), ...mappedRenewals].sort(
        (a, b) =>
  ...
  const metrics = useMemo(() => {
    ...
    const resellerRevenue = salesReseller.reduce(
      (acc, m) => acc + (m.total_brl || 0),
      0,
    );
    const resellerCredits = salesReseller.reduce(
      (acc, m) => acc + (m.qty_credits || 0),
      0,
    );

    const clientRevenue = salesClient.reduce(
      (acc, m) => acc + (m.total_brl || 0),
      0,
    );
    const clientCredits = salesClient.reduce(
      (acc, m) => acc + (m.qty_credits || 0),
      0,
    );
    ...
    const totalRestockCost = purchases.reduce(
      (acc, m) => acc + (m.total_brl || 0),
      0,
    );
  ...
  if (!server)
    return <div className="text-rose-500 p-8">Servidor não encontrado.</div>;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-3 pb-1 mb-6 border-b border-border">
        ...
        <div className="flex gap-3 w-full md:w-auto justify-end">
          ...
          <button
            ...
            className="h-10 px-5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-sm shadow-lg shadow-emerald-900/20 transition-all flex items-center gap-2"
          >
            ...
            <table className="w-full text-sm text-left relative border-collapse">
              <thead className="bg-transparent text-muted-foreground border-b border-border sticky top-0 z-10 backdrop-blur-md">
                <tr>
                  ...
                  <th className="px-5 py-3 font-medium text-[11px] uppercase tracking-wider text-center">
                    Qtd.
                  </th>
                  <th className="px-5 py-3 font-medium text-[11px] uppercase tracking-wider">
                    Valor (brl)
