...
function ToggleLine({
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-10 h-5 rounded-full border relative transition-colors ${value ? "bg-emerald-500/60 border-emerald-500/50" : "bg-white/10 border-slate-300 dark:border-white/20"}`}
      >
        <div
          className={`absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white transition-all ${value ? "left-[22px]" : "left-[4px]"}`}
        />
      </div>
    </button>
  );
...
export default function ResellerFormModal({
  ...
  const [primaryCountryLabel, setPrimaryCountryLabel] =
    useState("Brasil (+55)");
    ...
    try {
      ...
      if (json.exists && json.jid) {
        ...
        if (jidDigits) {
          ...
          setPrimaryPhoneRaw(
            inferred.formattedNational || inferred.nationalDigits || "",
          );
        }
      }
    } catch {
    ...
    (async () => {
      ...
      if (resellerToEdit) {
        ...
        const displayName = String(
          resellerToEdit.display_name ?? resellerToEdit.name ?? "",
        ).trim();
        setName(displayName);
        ...
        setDontMessageUntil(
          toDatetimeLocalValue(
            resellerToEdit.whatsapp_snooze_until ??
              resellerToEdit.dont_message_until ??
              null,
          ),
        );
        ...
        // --- TELEFONES EXTRAS ---
        let extraRaw =
          resellerToEdit.whatsapp_extra ??
          resellerToEdit.whatsapp_secondary ??
          [];
  ...
  function handlePrimaryValidate() {
    ...
    const finalUser =
      whatsUserTouched && whatsappUsername.trim()
  ...
  function addExtra() {
    setExtras((prev) => [
      ...prev,
      { id: Date.now(), e164: "", display: "", username: "", confirmed: false },
    ]);
  }
    ...
    setExtras((prev) =>
      prev.map((e) => {
        ...
        return {
          ...
          e164: inferred.e164,
          display:
            inferred.formattedNational || inferred.nationalDigits || e.display,
          confirmed: true,
        };
      }),
    );
  ...
  const errors = useMemo(() => {
    ...
    const rawPrimaryDigits = (primaryPhoneRaw || "").replace(/\D+/g, "");
    if (rawPrimaryDigits && rawPrimaryDigits.length < 8)
      out.push("WhatsApp principal inválido.");
    return out;
  ...
  async function handleSave() {
    ...
    try {
      ...
      const ddi = rawPrimaryDigits
        ...
        : "55";
      const natDigits = rawPrimaryDigits.startsWith(ddi)
      ...
      const payload = {
        ...
        whatsapp_opt_out_until: dontMessageUntil
      ...
      if (!isEditing) {
        ...
        const resellerIdToUse = String(
          (data as any)?.reseller_id ?? (data as any)?.id ?? data,
        );
      ...
      const { error: phoneErr } = await supabaseBrowser.rpc(
        "set_reseller_phones",
        {
          p_tenant_id: tenantId,
          p_reseller_id: resellerId,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-hidden overscroll-contain animate-in fade-in duration-200">
      <div className="w-full max-w-2xl max-h-[90vh] bg-white dark:bg-card border border-slate-200 dark:border-border rounded-xl shadow-2xl flex flex-col overflow-hidden min-h-0 transition-colors max-h-[90dvh]">
        {/* HEADER */}
