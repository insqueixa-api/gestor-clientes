...
const [showEditModal, setShowEditModal] = useState(false);
  ...
  // ✅ NOVO: Estado para o reset de acesso
  ...
  const handleResetAccess = async () => {
    ...
    const ok = await confirm({
      title: "Resetar Acesso do Portal",
      subtitle: "Isso inativará o link atual e redefinirá a senha para os 4 últimos dígitos do WhatsApp.",
      tone: "rose",
      icon: "🔄",
      details: [
        `Cliente: ${client.client_name}`,
        "Deseja enviar o novo link no WhatsApp?",
      ],
      confirmText: "Sim, enviar WhatsApp",
      cancelText: "Resetar sem enviar", // Vamos usar o cancelamento aqui como um "Não enviar", para não quebrar a UI
    });

    // Se o usuário clicar fora do modal (undefined), não faz nada. 
    // Se clicar no botão primário (true), manda WhatsApp. Se clicar no secundário (false), não manda.
    if (ok === undefined) return; 

    setIsResetting(true);
    try {
      const tid = await getCurrentTenantId();
      const res = await fetch("/api/admin/client-portal/reset-access", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          tenant_id: tid, 
          client_id: client.id,
          send_whatsapp: ok // true (com zap) ou false (sem zap)
        })
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Falha ao resetar acesso");
      
      addToast("success", "Acesso Resetado", "Link alterado e senha redefinida!");
    } catch (e: any) {
      addToast("error", "Erro ao resetar", e.message || "Ocorreu um erro no servidor.");
    } finally {
...
{/* Botões de Ação (Responsivos) */}
      <div className="flex items-center gap-2 shrink-0">
        ...
        {/* ✅ NOVO: Botão Resetar Portal */}
        {!client.client_is_archived && (
          <button
            onClick={handleResetAccess}
            disabled={isResetting}
            title="Resetar link do Portal"
            className="h-9 w-9 sm:w-auto px-0 sm:px-3 rounded-lg border border-indigo-500/20 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 font-bold text-xs hover:bg-indigo-500/20 transition-all shadow-sm flex items-center justify-center gap-2 shrink-0"
          >
            {isResetting ? <span className="animate-spin text-sm">⏳</span> : <IconSync />}
            <span className="hidden sm:inline">Resetar Portal</span>
          </button>
        )}
