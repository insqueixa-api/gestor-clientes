...
function ddiMeta(ddi: string) {
  ...
  if (!opt)
    return { label: `DDI Desconhecido (+${ddi})`, pretty: `🌍 DDI (+${ddi})` }; // ✅ Mais amigável
  return {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        <div className="h-10 min-w-[160px] px-3 bg-slate-100 dark:bg-black/30 border border-border rounded-lg flex items-center text-xs font-medium text-foreground/90">
          {countryLabel || "—"}
        </div>
...
export default function NovoCliente({
  ...
  const [primaryCountryLabel, setPrimaryCountryLabel] = useState<string>(
    ddiMeta("55").label,
  );
  ...
  const [secondaryCountryLabel, setSecondaryCountryLabel] = useState<string>(
    ddiMeta("55").label,
  );
  const [secondaryWhatsappUsername, setSecondaryWhatsappUsername] =
    useState("");
  const [secondaryWhatsUserTouched, setSecondaryWhatsUserTouched] =
    useState(false);
  ...
  const [secondaryWaValidation, setSecondaryWaValidation] =
    useState<WaValidation>(null);
  const waValidateTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const secondaryValidateTimer = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  async function validateWa(
    username: string,
    setter: (v: WaValidation) => void,
  ...
  const [registerRenewal, setRegisterRenewal] = useState(
    () => !isTrialMode && !isEditing,
  );
  ...
  function handleDonePrimary() {
    ...
    const finalUser = whatsUserTouched
      ? whatsappUsername
  ...
  function handleDoneSecondary() {
    ...
    const finalUser = secondaryWhatsUserTouched
    ...
    void validateWa(
      finalUser,
      setSecondaryWaValidation,
      setSecondaryCountryLabel,
    );
  }
      ...
      try {
        ...
        if (tmplErr) {
          ...
          if (isTrialMode) {
            ...
            const defaultTpl =
              list.find((t) =>
                (t.name || "").trim().toLowerCase().startsWith("teste"),
              ) ||
              list.find((t) =>
                (t.name || "").toLowerCase().includes("teste"),
              ) ||
              null;
          ...
          if (!isTrialMode) {
            ...
            const defaultTpl =
              list.find((t) =>
                (t.name || "").trim().toLowerCase().includes("pagamento"),
              ) ||
        ...
        if (initialTableId) {
          ...
          const t0 =
            allTables.find((t) => t.id === initialTableId) ||
            defaultBRL ||
            null;
          if (t0) {
        ...
        if (clientToEdit) {
          ...
          if (clientToEdit.whatsapp_e164) {
            ...
            if (!whatsUserTouched) {
              setWhatsappUsername(
                clientToEdit.whatsapp_username ||
                  onlyDigits(clientToEdit.whatsapp_e164),
              );
            }
          }
          ...
          if (
            clientToEdit.secondary_display_name ||
            clientToEdit.secondary_phone_e164 ||
            clientToEdit.secondary_whatsapp_username
          ) {
            setShowSecondary(true);
          }

          if (clientToEdit.secondary_phone_e164) {
            const { ddi, national } = splitE164(
              clientToEdit.secondary_phone_e164,
            );
            ...
            if (!secondaryWhatsUserTouched) {
              setSecondaryWhatsappUsername(
                clientToEdit.secondary_whatsapp_username ||
                  onlyDigits(clientToEdit.secondary_phone_e164),
              );
            }
          ...
          if (clientToEdit.price_amount != null) {
            setPlanPrice(
              Number(clientToEdit.price_amount).toFixed(2).replace(".", ","),
            );
            ...
            const currentTableId =
              clientPlanTableId &&
              allTables.some((t) => t.id === clientPlanTableId)
                ? clientPlanTableId
                : initialTableId || "";
            const tSel = allTables.find((t) => t.id === currentTableId) || null;
            if (tSel) {
              const pAuto = pickPriceFromTable(
                tSel,
                foundPeriod,
                clientToEdit.screens || 1,
              );
              setPlanPrice(
                Number(pAuto || 0)
                  .toFixed(2)
          ...
          if (clientToEdit.dont_message_until) {
            const v = isoToLocalDateTimeInputValue(
              clientToEdit.dont_message_until,
            );
          ...
          try {
            if (clientToEdit.id) {
              ...
              if (!nerr) {
                ...
                setExternalUserId(String(nrow?.external_user_id || "").trim()); // ✅ SE ACHOU A DATA NO BANCO, ATUALIZA A TELA AQUI:
                if (nrow?.created_at) {
                ...
                setExternalUserId(
                  String((clientToEdit as any)?.external_user_id || "").trim(),
                );
              }
            } else {
              setNotes(clientToEdit.notes || "");
            ...
            if (currentApps) {
              const instances = currentApps.map((ca: any) => {
                ...
                for (const f of cfg) {
                  ...
                  const v =
                    idKey && restValues[idKey] != null
                      ? restValues[idKey]
                      : labelKey && restValues[labelKey] != null
                        ? restValues[labelKey]
                        : "";
  ...
  useEffect(() => {
    if (trialProvider === "ELITE" && selectedPlanPeriod === "ANNUAL") {
      ...
      addToast(
        "warning",
        "Limite",
        "A Elite permite recargas de no máximo 6 meses.",
      );
    }
  ...
  function updateAppConfig(
    instanceId: string,
    key: "costType" | "partnerServerId" | "auto_configure",
    value: any,
  ) {
  ...
  function getMacFromApp(appInstance?: SelectedAppInstance) {
    ...
    let macValue = "";
    const macField = appInstance.fields_config?.find(
      (f: any) => String(f?.type || "").toUpperCase() === "MAC",
    );
    if (macField) {
    ...
    if (!macValue) {
      const foundKey = Object.keys(appInstance.values).find((k) =>
        String(appInstance.values[k]).includes(":"),
      );
  ...
  async function handleConfigApp(instanceId: string) {
    if (!clientToEdit?.id) {
      addToast(
        "warning",
        "Atenção",
        "Salve o cliente primeiro antes de configurar o aplicativo.",
      );
      return;
    }
    ...
    if (!handler) {
      addToast(
        "error",
        "Erro de Rota",
        `Não sabemos como integrar o app "${appName}". Nenhuma regra definida.`,
      );
      return;
    }
  ...
  async function handleDeleteApp(instanceId: string) {
    ...
    if (!handler) {
      addToast(
        "error",
        "Erro de Rota",
        `Integração não configurada para o app "${appName}". Impossível deletar.`,
      );
      return;
    }
    ...
    if (handler.actionPrefix === "GERENCIAAPP") {
      ...
      if (dateField) {
        const fieldKey = dateField.id || dateField.label;
        if (fieldKey)
          updateAppFieldValue(currentApp!.instanceId, String(fieldKey), "");
      }
    }
      ...
      try {
        ...
        if (apiJson?.ok)
          ...
          addToast(
            "error",
            "Não Removido",
            apiJson?.error || "Falha ao apagar no painel.",
          );
      } catch (err: any) {
        setLoading(false);
        setLoadingStep("");
  ...
  async function executeSave() {
    ...
    try {
      ...
      const ddi = rawPrimaryDigits
        ? extractDdiFromLabel(primaryCountryLabel)
        : "55";
      const nationalDigits = rawPrimaryDigits.startsWith(ddi)
        ? rawPrimaryDigits.slice(ddi.length)
        : rawPrimaryDigits;
      const finalPrimaryE164 = rawPrimaryDigits
        ? `+${ddi}${nationalDigits}`
        : "";
      ...
      if (rawSecondaryDigits) {
        ...
        const nat2 = rawSecondaryDigits.startsWith(ddi2)
          ? rawSecondaryDigits.slice(ddi2.length)
          : rawSecondaryDigits;
        finalSecondaryE164 = `+${ddi2}${nat2}`;
      }

      const secName = secondaryName.trim() ? secondaryName.trim() : null;
      const secPrefix = secondarySalutation?.trim()
        ? secondarySalutation.trim()
        : null;
      const secWhatsUser = secondaryWhatsappUsername.trim()
        ? secondaryWhatsappUsername.trim()
        : null;
      ...
      if (technology === "Personalizado")
        finalTechnology = customTechnology.trim(); // ✅ Converte a data de cadastro para envio

      const finalCreatedAt = createdAt ? localDateTimeToISO(createdAt) : null; // Dados do RPC
      ...
      const rpcPlanLabel = isTrialMode
        ? PLAN_LABELS["MONTHLY"]
        : PLAN_LABELS[selectedPlanPeriod];
      ...
      if (isEditing && clientId) {
        ...
        if (isTrialMode && syncWithServer && serverId) {
          ...
          try {
            ...
            if (srv?.panel_integration) {
              ...
              if (integErr)
                throw new Error(
                  "Erro ao buscar integração: " + integErr.message,
                );
              ...
              if (provider === "ELITE") {
                ...
                if (!apiUrl) {
                  throw new Error(
                    "Provider não suportado para integração automática de testes.",
                  );
                }
                ...
                if (sessErr)
                  throw new Error(`Sessão inválida: ${sessErr.message}`);
                if (!token)
                  throw new Error(
                    "Sem sessão ativa. Recarregue a página e faça login novamente.",
                  );
                ...
                const nextUsername =
                  apiData?.username != null ? String(apiData.username) : "";
                const nextPassword =
                  apiData?.password != null ? String(apiData.password) : "";
                const nextM3u =
                ...
                const nextExternalUserId = String(
                  nextExternalUserIdRaw || "",
                ).trim();
                ...
                if (syncUrl) {
                  ...
                  if (!syncRes.ok) {
                    ...
                    if (syncData?.expires_at_iso)
                      apiVencimento = syncData.expires_at_iso;
                  }
                }
        ...
        if (clientId) {
          ...
          await supabaseBrowser.from("client_events").insert({
            ...
            meta: {
              ...
              ...(isTrialMode
                ? { trial_hours: testHours }
                : {
                    screens: rpcScreens,
                    months: PLAN_MONTHS[selectedPlanPeriod],
                  }),
            },
          });
        }
        ...
        finalM3u = (
          apiM3uUrl ||
          m3uUrl ||
          buildM3uUrlSilent(apiUsername, apiPassword) ||
          ""
        ).trim();

        const finalExternalUserId = (
          apiExternalUserId ||
          externalUserId ||
          ""
        ).trim();
        ...
        if (selectedApps.length > 0 && clientId) {
          ...
          if (!isEditing) {
            for (const app of selectedApps) {
              ...
              const catAppForAuto = catalog.find(
                (c) => c.id === app.app_id,
              ) as any;
              const appIntegKeyAuto =
                String(catAppForAuto?.integration_type || "")
                  .trim()
                  .toUpperCase() ||
                handler?.actionPrefix?.toUpperCase() ||
                "";
              const appIntegData = appIntegrations.find(
                (a) => a.app_name.toUpperCase() === appIntegKeyAuto,
              );
              const appBaseUrl = appIntegData?.api_url || "";
  ...
  async function handleViewDns() {
    if (serverDomains.length === 0) {
      addToast(
        "warning",
        "Sem DNS",
        "Este servidor não possui DNS configurada.",
      );
      return;
    }
    const selectedServerName =
  ...
  function handleSave() {
    ...
    if (technology === "Personalizado" && !customTechnology.trim()) {
      addToast(
        "error",
        "Tecnologia",
        "Para 'Personalizado', digite o nome da tecnologia.",
      );
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-muted/50 rounded-t-xl shrink-0">
            ...
            <button
              onClick={onClose}
              type="button"
              className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 text-muted-foreground/80 dark:text-muted-foreground transition-colors"
            >
              <IconX />
            </button>
          </div>
          ...
          <div className="flex justify-center border-b border-border bg-muted/50/50 dark:bg-white/5 px-4 py-2">
              ...
              {(
                ...
                <button
                  ...
                  className={`flex-1 sm:flex-none px-6 py-2 text-xs font-medium rounded-md transition-all uppercase tracking-wider whitespace-nowrap ${
                    activeTab === tab.key
                      ? "bg-card dark:bg-white/10 text-emerald-600 dark:text-emerald-400 shadow-sm"
          ...
          <div className="p-3 sm:p-4 overflow-y-auto overscroll-contain space-y-3 flex-1 min-h-0 bg-card custom-scrollbar">
            ...
            {activeTab === "dados" && (
              <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setPapaTesteInfo(null);
                          if (waValidateTimer.current)
                            clearTimeout(waValidateTimer.current);
                          waValidateTimer.current = setTimeout(() => {
                            void validateWa(
                              val,
                              setWaValidation,
                              setPrimaryCountryLabel,
                            );
                      ...
                      <div className="mt-1 flex items-center gap-1.5">
                        {papaTesteLoading ? (
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              await confirm({
                                ...
                                details: lines.map((line, i) => (
                                  <div
                                    key={i}
                                    className="text-xs text-foreground/90/80 py-1 border-b border-slate-100 dark:border-border last:border-0"
                                  >
                                    {line}
                                  </div>
                                )),
                            ...
                            className="gap-1 px-2 py-1 rounded-lg text-[10px] font-medium tracking-tight shadow-sm bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors flex items-center gap-1"
                          >
                ...
                {!showSecondary ? (
                  <div className="pt-2 mt-4 border-t border-border flex justify-between items-center">
                    <Label> </Label>
                    <button
                      type="button"
                  ...
                  <div className="pt-2 mt-4 border-t border-border space-y-4 animate-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      <div className="col-span-1">
                        <Label>Saudação</Label>
                        <Select
                          value={secondarySalutation}
                          onChange={(e) =>
                            setSecondarySalutation(e.target.value)
                          }
                        >
                          <option value=""> </option>
                          <option value="Sr.">Sr.</option>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (secondaryValidateTimer.current)
                                clearTimeout(secondaryValidateTimer.current);
                        ...
                        {secondaryWaValidation && (
                          <div
                            className={`mt-1.5 flex items-center gap-1.5 text-[11px] font-medium ${secondaryWaValidation.loading ? "text-muted-foreground/80" : secondaryWaValidation.exists ? "text-emerald-600 dark:text-emerald-400" : "text-rose-500"}`}
                          >
                    ...
                    <div className="pt-2 pb-1 border-b border-border"></div>
                  </div>
                )}
 ...
 className="flex-1 text-xs "
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            addToast(
                              "success",
                              "Copiado!",
                              "Link M3U copiado para a área de transferência.",
                            );
                          }}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      <div className="col-span-2 sm:col-span-1">
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setSelectedPlanPeriod(e.target.value as any)
                          }
                        >
                          {Object.entries(PLAN_LABELS)
                            .filter(([k]) => {
                              ...
                              if (trialProvider === "ELITE" && k === "ANNUAL")
                                return false;
                    ...
                    {isTrialMode && (
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          value={testHours}
                          onChange={(e) =>
                            setTestHours(Number(e.target.value) as 2 | 4 | 6)
                          }
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-2 rounded-lg bg-slate-200 dark:bg-white/10 text-[10px] font-medium text-muted-foreground dark:text-muted-foreground hover:bg-slate-300 dark:hover:bg-white/20 border border-border whitespace-nowrap"
                          title="Fim do dia"
                        >
                          23:59
                        </button>
                    ...
                    <>
                      {isTrialMode ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 items-stretch">
                          ...
                          >
                            <div className="flex items-center justify-between w-full">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <span
                                    className={`text-sm font-medium block ${syncWithServer ? "text-sky-700 dark:text-sky-400" : "text-muted-foreground"}`}
                                  >
                                    Teste Automático
                                  </span>
                              ...
                              <Switch
                                ...
                                onChange={(v) =>
                                  hasIntegration && setSyncWithServer(v)
                                }
                                label=""
                              />
                            </div>
                          ...
                          <div className="p-3 rounded-xl border border-border bg-card dark:bg-white/5 flex flex-col gap-3">
                            <div
                              onClick={() => setSendTrialWhats(!sendTrialWhats)}
                              className="h-10 px-3 rounded-lg border border-border bg-muted/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 transition-colors flex items-center justify-between gap-3 shrink-0"
                            >
                              ...
                              <Switch
                                checked={sendTrialWhats}
                                onChange={setSendTrialWhats}
                                label=""
                              />
                            </div>
                        ...
                        <div className="flex flex-col gap-3 pt-2">
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            ...
                            {(isEditing || isTrialMode) && (
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    <span
                                      className={`text-xs font-medium block ${syncWithServer ? "text-sky-700 dark:text-sky-400" : "text-muted-foreground"}`}
                                    >
                                      Sincronizar Painel
                                    </span>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <span
                                    className={`text-xs font-medium block ${registerRenewal ? "text-emerald-700 dark:text-emerald-400" : "text-muted-foreground"}`}
                                  >
                                    Registrar Financeiro
                                  </span>
                          ...
                          <div className="p-3 rounded-xl border border-border bg-card dark:bg-white/5">
                            <div
                            ...
                            >
                              <div
                                ...
                                className="h-10 px-3 rounded-lg border border-border bg-muted/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 transition-colors flex items-center justify-between gap-3"
                              >
                                <span className="text-xs font-medium text-muted-foreground dark:text-muted-foreground">
                                  Enviar msg pagto?
                                </span>
                                ...
                                <Switch
                                  checked={sendPaymentMsg}
                                  onChange={setSendPaymentMsg}
                                  label=""
                                />
                              </div>
                        ...
                        <div className="flex justify-between items-center">
                          <div
                            ...
                            onClick={() =>
                              setSelectedApps((prev) =>
                                prev.map((a) =>
                                  a.instanceId === app.instanceId
                                    ? { ...a, is_minimized: !a.is_minimized }
                                    : a,
                                ),
                              )
                            }
                          >
                            <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 uppercase tracking-wider flex items-center gap-2">
                              ...
                              {hasInteg && (
                                <span
                                  title={`Integração: ${integrationType}`}
                                  className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-sky-100 dark:bg-sky-500/15 border border-sky-200 dark:border-sky-500/30 text-sky-600 dark:text-sky-400"
                                >
                                  <svg
                                    width="10"
                                    height="10"
                            ...
                            {isExpiringSoon && diffDays !== null && (
                              <span
                                ...
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-rose-100 dark:bg-rose-500/15 border border-rose-200 dark:border-rose-500/30 text-rose-600 dark:text-rose-400 animate-pulse shadow-sm ml-1"
                              >
                                <svg
                                  width="10"
                                  height="10"
                                ...
                                {isEditing ? (
                                  ...
                                  <div className="grid grid-cols-3 gap-2">
                                    ...
                                    <button
                                      ...
                                      className="h-10 rounded-lg bg-slate-100 dark:bg-white/10 border border-border text-muted-foreground dark:text-muted-foreground hover:bg-slate-200 dark:hover:bg-white/20 transition-colors flex items-center justify-center gap-1.5"
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      <svg
                                        className={`w-3.5 h-3.5 shrink-0 ${app.auto_configure !== false ? "text-sky-600 dark:text-sky-400" : "text-muted-foreground/80"}`}
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                        strokeWidth="2"
                                      ...
                                      <span
                                        className={`text-xs font-medium ${app.auto_configure !== false ? "text-sky-700 dark:text-sky-400" : "text-muted-foreground"}`}
                                      >
                                ...
                                app.fields_config.map(
                                  (field: any, index: number) => {
                                    ...
                                    const safeKey =
                                      fieldKey ||
                                      rawLabel ||
                                      `${app.instanceId}-${index}`;
                                    const isDateField = field?.type === "date";

                                    const fieldValue =
                                      (fieldKey &&
                                      (app.values as any)?.[fieldKey] != null
                                        ? String((app.values as any)[fieldKey])
                                        : "") ||
                                      (label &&
                                      (app.values as any)?.[label] != null
                ...
                <div className="relative mb-4">
                  {!showAppSelector ? (
                    <button
                      ...
                      className="w-full h-14 border-2 border-dashed border-border dark:border-border rounded-xl text-muted-foreground dark:text-white/60 hover:text-emerald-600 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all font-medium text-sm uppercase flex items-center justify-center gap-2"
                    >
                          ...
                          return filtered.map((app) => {
                            ...
                            return (
                              <button
                                ...
                                className="w-full text-left px-4 py-3 text-sm text-foreground/90 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-400 border-b border-slate-50 dark:border-border last:border-0 transition-colors flex items-center justify-between group"
                              >
                                <div className="flex items-center gap-2">
          ...
          <div className="px-6 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] border-t border-border bg-muted/50 flex justify-end gap-2 rounded-b-xl shrink-0">
            <button
              onClick={onClose}
              type="button"
              className="px-4 py-2 rounded-lg border border-border text-muted-foreground dark:text-white/60 hover:bg-slate-200 dark:hover:bg-white/10 text-xs font-medium transition-colors"
            >
              Cancelar
            </button>
          ...
          <div className="w-full max-w-sm bg-card border border-border rounded-xl shadow-2xl p-6 flex flex-col gap-5 overflow-hidden min-h-0 max-h-[90vh] animate-in fade-in zoom-in-95 duration-200 max-h-[90dvh]">
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 h-12 rounded-xl border border-border text-muted-foreground dark:text-muted-foreground/80 font-medium text-sm hover:bg-muted/50 dark:hover:bg-white/5 transition-colors"
              >
                Voltar
              </button>
