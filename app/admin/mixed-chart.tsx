...
function MixedChartBase({ data, heightClass = "h-80", formatValue }: MixedChartProps) {
  ...
  const { maxNice, ticks } = useMemo(() => {
    ...
    const niceCeil = (n: number) => {
      ...
      const f = n / exp;
      const nice =
        f <= 1 ? 1 : f <= 1.2 ? 1.2 : f <= 1.5 ? 1.5 : f <= 2 ? 2 :
        f <= 2.5 ? 2.5 : f <= 3 ? 3 : f <= 4 ? 4 : f <= 5 ? 5 :
        f <= 6 ? 6 : f <= 8 ? 8 : 10;
    ...
    <div className={`relative w-full ${heightClass}`} ref={chartRef}>
      ...
      {ticks.map((t, i) => {
        ...
        return (
          <div
            ...
            style={{ left: 0, top: y - 6, width: paddingLeft - 8, textAlign: "right", zIndex: 10 }}
          >
            <span className="sv tabular-nums text-[9px] sm:text-[10px]" style={{ color: textColor }}>
              {fmt(t)}
            </span>
          </div>
        );
      ...
      >
        ...
        {data.map((d, i) => {
          ...
          const yB1 = getY(d.bar1); const hB1 = y0 - yB1;
          const yB2 = getY(d.bar2); const hB2 = y0 - yB2;

          return (
            <g key={i}>
              {d.bar1 !== undefined && hB1 > 0 && (
                <rect x={cx - barWidth - 2} y={yB1} width={barWidth} height={hB1}
                  fill={bar1Color} rx="3" opacity="0.75" />
              )}
              {d.bar2 !== undefined && hB2 > 0 && (
                <rect x={cx + 2} y={yB2} width={barWidth} height={hB2}
                  fill={bar2Color} rx="3" opacity="0.75" />
              )}
              <text
                x={cx} y={y0 + 18}
        ...
        {data.length > 0 && (
          <>
            {/* Glow */}
            <path d={pathLine1} fill="none" stroke={line1Color} strokeWidth="5"
              strokeLinecap="round" strokeLinejoin="round" opacity="0.12"
              style={{ filter: "blur(4px)" }} />
            <path d={pathLine2} fill="none" stroke={line2Color} strokeWidth="5"
              strokeLinecap="round" strokeLinejoin="round" opacity="0.12"
              style={{ filter: "blur(4px)" }} />

            {/* Linhas principais */}
            <path d={pathLine1} fill="none" stroke={line1Color} strokeWidth="2.5"
              strokeLinecap="round" strokeLinejoin="round" />
            <path d={pathLine2} fill="none" stroke={line2Color} strokeWidth="2.5"
              strokeLinecap="round" strokeLinejoin="round" />
            ...
            {data.map((d, i) => (
              <g key={`dots-${i}`}>
                {d.line1 !== undefined && (
                  <circle cx={getX(i)} cy={getY(d.line1)} r="3.5"
                    fill={bgSurface} stroke={line1Color} strokeWidth="2.5"
                    className="pointer-events-none" />
                )}
                {d.line2 !== undefined && (
                  <circle cx={getX(i)} cy={getY(d.line2)} r="3.5"
                    fill={bgSurface} stroke={line2Color} strokeWidth="2.5"
                    className="pointer-events-none" />
                )}
              </g>
            ))}
