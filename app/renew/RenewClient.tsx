...
// ✅ REGRA BLINDADA: Lê a moeda direto da conta do cliente (assume BRL se vier vazio por segurança)
    const isInternational = (selectedAccount?.price_currency || "BRL") !== "BRL";
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-y-auto max-h-[95vh]">
          ...
          {/* ========================================================= */}
          {/* ONLINE - PIX (Somente BRL) */}
          {/* ========================================================= */}
          {isOnline && !isApproved && !isRejected && !isInternational && (
            <>
          ...
          {/* ========================================================= */}
          {/* MANUAL - (PIX BRL ou IBAN/SWIFT Internacional) */}
          {/* ========================================================= */}
          {(isManual || (isOnline && isInternational)) && !isApproved && !isRejected && (
            <>
              {/* Header Dinâmico */}
              ...
              <div className="px-5 pt-4 pb-3 space-y-3">
                ...
                <div className="space-y-3">
                    ...
                    <>
                      <div>
                        ...
                        <div className="relative group">
                          <input
                            type="text"
                            value={paymentData.pix_key || paymentData.chave_pix_manual}
                            readOnly
                          ...
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(paymentData.pix_key || paymentData.chave_pix_manual);
                              setCopiedKey(true);
                              setTimeout(() => setCopiedKey(false), 3000);
                            }}
                            className={`absolute right-1 top-1 bottom-1 px-4 text-white font-bold text-xs rounded-md transition-all flex items-center justify-center gap-1.5 min-w-[90px] ${
                      ...
                      {/* Nome do Titular e Banco (Apenas para Brasil) */}
                      {paymentData.holder_name && (
                        <div>
                  ...
                  {/* Valor Fixo para ambos */}
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Valor</p>
                    <p className={`text-2xl font-bold ${isInternational ? 'text-indigo-900' : 'text-slate-800'}`}>
                ...
                {paymentData.instructions && (
                  <div className={`p-3 border rounded-xl ${isInternational ? 'bg-indigo-50 border-indigo-200' : 'bg-violet-50 border-violet-200'}`}>
                    <p className={`text-xs ${isInternational ? 'text-indigo-700' : 'text-violet-700'}`}>
                      {paymentData.instructions}
                    </p>
                  </div>
                )}

                {/* Botão WhatsApp Comum para Envio de Comprovante */}
                {waNumber ? (
                  <a
                    href={`https://wa.me/${waNumber}?text=${isInternational ? 'Hello, I just made my payment!' : 'Olá, acabei de realizar o pagamento!'}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-2 py-3 bg-[#25D366] text-white font-bold rounded-xl hover:bg-[#20BA5A] transition-colors mt-2"
                  >
                    ...
                    {isInternational ? 'Send Receipt' : 'Enviar Comprovante'}
                  </a>
                ) : null}
