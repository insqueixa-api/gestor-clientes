...
//      → {status:"success", token (JWT, Bearer), device:{_id, expire_date, ...}}
//   4. POST /frontend/device/playlists        {device_id} (Bearer)
//      → {playlists:[{_id, playlist_name, is_protected}], device:{...expire_date}}
//   5. POST /frontend/device/savePlaylist     {playlist_name, playlist_url, playlist_type:"general",
//      protect, pin, device_id, ...} (Bearer)
...
// trocar de domínio, sem precisar mexer em código.
...
async function messiTvLogin(
  ...
  throw new Error(`Falha no login do MessiTV (mac/device key ou captcha): ${lastError}`);
}
...
async function listPlaylists(
  ...
  authToken: string,
  deviceId: string,
): Promise<{ playlists: any[]; device: any }> {
...
async function savePlaylist(
  ...
  if (res.status !== 200 || json?.status !== "success") {
    throw new Error(json?.msg || json?.message || `Falha ao criar playlist no MessiTV (status ${res.status}).`);
  }
}
...
async function deletePlaylistByName(
  ...
  const { playlists } = await listPlaylists(siteRoot, authToken, deviceId);
  if (!playlists.length) {
  ...
  try {
    ...
    if (!deviceKey) {
      return NextResponse.json(
        { ok: false, error: "Device Key é obrigatório pro MessiTV — preencha o campo 'Device Key' desse app." },
        { status: 400 },
      );
    }
    ...
    if (integErr || !integ?.api_url) {
      return NextResponse.json({ ok: false, error: "Integração MessiTV não configurada." }, { status: 500 });
    }
    ...
    if (action === "create") {
      ...
      const { authToken, device } = await messiTvLogin(siteRoot, macValue, deviceKey, geminiKey);
    ...
    if (action === "delete") {
      ...
      try {
        await deletePlaylistByName(siteRoot, authToken, device._id, searchName, pin);
      } catch (e: any) {
        if (e?.notFound) {
