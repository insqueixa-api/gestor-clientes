...
export async function GET() {
  // URL atualizada para o servidor Elite
  const M3U_URL = "http://chinax.asia/get.php?username=InsqueixaElite&password=811165216126xx&type=m3u_plus&output=ts";
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Falha ao baixar M3U Elite: HTTP ${response.status}`);
    }
