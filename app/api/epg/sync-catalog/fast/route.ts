...
const M3U_RAW_KEY = "catalog/fast_raw.m3u";    // M3U temporário no R2
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[CATALOG-FAST] Upsert availability: ${availRows.length}...`);
    for (let i = 0; i < availRows.length; i += BATCH) {
      ...
      if (error) console.error(`[CATALOG-FAST] Erro availability lote ${i}:`, error.message);
    }
    ...
    console.log(`[CATALOG-FAST] Upsert episodes: ${episodeRows.length}...`);
