...
//
// Cron de sincronização do catálogo — Servidor FAST
//
// Fluxo:
...
//
// Endereço: POST /api/epg/sync-catalog/fast
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[CATALOG-FAST] Baixando M3U de ${dns[0]}...`);
    ...
    for (const d of dns) {
      try {
        ...
        console.warn(`[CATALOG-FAST] DNS ${d} falhou: ${e.message}`);
      }
    }
    ...
    // ── 3. Parsear M3U ────────────────────────────────────────────────────────
    console.log(`[CATALOG-FAST] Parseando ${m3uText.length} bytes...`);
    ...
    console.log(`[CATALOG-FAST] Parse concluído:`, stats);
    ...
    console.log(`[CATALOG-FAST] Upsert catalog_master: ${todasMaster.length} títulos...`);
    for (let i = 0; i < todasMaster.length; i += BATCH_MASTER) {
      ...
      if (error) console.error(`[CATALOG-FAST] Erro master lote ${i}:`, error.message);
    }

    // ── 4b. Buscar IDs gerados/existentes no catalog_master ───────────────────
    console.log(`[CATALOG-FAST] Buscando IDs do catalog_master...`);
    ...
    for (let i = 0; i < titulos.length; i += 500) {
      ...
      if (error) {
        console.error(`[CATALOG-FAST] Erro ao buscar IDs lote ${i}:`, error.message);
      } else {
    ...
    console.log(`[CATALOG-FAST] Upsert catalog_availability: ${availabilityRows.length} entradas...`);
    ...
    for (let i = 0; i < availabilityRows.length; i += BATCH_AVAILABILITY) {
      ...
      if (error) {
        console.error(`[CATALOG-FAST] Erro availability lote ${i}:`, error.message);
      } else {
    ...
    console.log(`[CATALOG-FAST] Upsert catalog_episodes: ${episodeRows.length} episódios...`);
    ...
    console.log(`[CATALOG-FAST] Atualizando contadores...`);
    ...
    await salvarLog(log);
    console.log(`[CATALOG-FAST] Concluído em ${duracao}s`, log.resultado);
