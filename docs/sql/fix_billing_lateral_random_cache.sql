-- Corrige o sorteio de variações de mensagem em billing_enqueue_scheduled
-- (docs/sql/billing_enqueue_scheduled_campaign_window.sql), que na prática
-- nunca sorteava nada — todo cliente elegível de uma mesma automação
-- recebia SEMPRE a mesma variação (às vezes o texto principal, às vezes
-- uma variação específica, mas sempre A MESMA pra todo mundo do lote).
--
-- Causa raiz (confirmada empiricamente, não é chute): é uma armadilha
-- conhecida do PostgreSQL com LATERAL + função volátil. O trecho:
--
--   cross join lateral (
...
--   ) picked
--
-- só referencia `a.message_template_id` como parâmetro correlacionado — e
-- esse valor é IDÊNTICO para todas as linhas de cliente de uma mesma
-- automação (é sempre a mesma automação `a`, só o cliente muda). O
-- planejador do Postgres decide se re-executa o lado interno de um LATERAL
-- olhando SÓ se os parâmetros correlacionados mudaram de valor entre uma
-- linha externa e outra — ele não sabe (nem verifica) que o corpo da
-- subquery contém uma função VOLATILE (random()). Como o parâmetro
-- (`message_template_id`) não muda ao longo de todo o lote de clientes da
-- mesma automação, o Postgres reaproveita o resultado já calculado (via um
-- nó Materialize) em vez de rodar `order by random()` de novo — ou seja,
-- sorteia UMA vez por automação/execução, não uma vez por cliente.
--
-- Reproduzido e comprovado direto no banco: rodando a mesma CTE isolada
-- pra ~219 clientes elegíveis de uma automação, os 219 vinham com a
-- EXATA MESMA variação. Trocando `order by random()` por
-- `order by md5(c.id::text || random()::text)` — que faz o `id` do
-- cliente (que varia linha a linha) entrar na conta do LATERAL, forçando
-- o Postgres a tratá-lo como parâmetro que mudou e reexecutar a
-- subconsulta — o mesmo teste passou a distribuir entre as 5-6 opções do
-- pool normalmente. `md5(...)` aqui não muda a aleatoriedade real (ainda
-- depende de random()), só impede o cache indevido.
--
-- Único trecho alterado: a linha `order by random()` dentro do
-- `cross join lateral` da CTE `impacted`. Todo o resto da função
-- (janela de disparo, filtros de elegibilidade, embaralhamento entre
-- automações, delay cumulativo) é idêntico ao último CREATE OR REPLACE
-- (billing_enqueue_scheduled_campaign_window.sql) — mantido aqui na
-- íntegra só porque CREATE OR REPLACE precisa do corpo inteiro.
...
begin
  ...
  impacted as (
    ...
    cross join lateral (
      ...
      from tpl_variants tv
      where tv.template_id = a.message_template_id
      -- ✅ FIX: md5(c.id::text || random()::text) em vez de só random() —
      -- ver comentário no topo do arquivo. c.id entrando na conta força o
      -- Postgres a reconhecer que o parâmetro correlacionado muda a cada
      -- linha de cliente, evitando o cache indevido do LATERAL.
